  ...
  BUILDINGS.forEach(bld => {
    ...
    section.innerHTML=`
      <div class="building-header">
        ...
        <span class="building-count">${group.length} hall${group.length!==1?'s':''}</span>
      </div>
...
function makeCard(h) {
  ...
  div.innerHTML=`
    ...
    <div class="card-body">
      <div class="card-label"><span class="card-label-txt">Lecture Hall</span>${badge}</div>
      <div class="card-num">#${h.hall_id}</div>
      <div class="card-meta">
...
window.goToBook = (hallId) => {
  ...
  setTimeout(()=>{ document.getElementById('bk-hall').value=hallId; updateHallPreview(hallId); },100);
};
window.goToHallSchedule = (hallId) => {
  document.querySelector('[data-view="schedule"]').click();
  setTimeout(()=>{ document.getElementById('sched-hall').value=hallId; loadSchedule(); },100);
};
